...
void processPhotoCells(photoVals *photoValues, bool direction) {
  ...
  if (direction) {
	  ...
	  (*photoValues).Left_Photo = map((*photoValues).Left_Photo, frontCalibration.LeftMin, frontCalibration.LeftMax, 0, 2); // Anything greater than 0 indicates a light surface, anything that is 0 indicates a dark surface
	  (*photoValues).Right_Photo = map((*photoValues).Right_Photo, frontCalibration.RightMin, frontCalibration.RightMax, 0, 2);
  }
  else {
	  (*photoValues).Left_Photo = constrain((*photoValues).Left_Photo, backCalibration.LeftMin, backCalibration.LeftMax);
	  	  ...
	  	  (*photoValues).Left_Photo = map((*photoValues).Left_Photo, backCalibration.LeftMin, backCalibration.LeftMax, 0, 2); // Anything greater than 0 indicates a light surface, anything that is 0 indicates a dark surface
	  	  (*photoValues).Right_Photo = map((*photoValues).Right_Photo, backCalibration.RightMin, backCalibration.RightMax, 0, 2);
  }
}
