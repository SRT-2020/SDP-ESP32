...
void telegram_task(void *pvParameters) {
    ...
    ESP_LOGW(TAG, "Begining COnfiguration");
    ...
    sprintf(post_data,"{\"chat_id\":%s,\"text\":\"%s\"}",chat_ID1, message);
...
static esp_err_t obstacle_handler(httpd_req_t *req)
{
    memset(message, 0, sizeof(message));
    strcpy(message, "Obstacle in Path. User Assistance Required");
...
{
    memset(message, 0, sizeof(message));
    strcpy(message, "Trash-E Disposal Routine Commencing");
    xTaskCreate(&telegram_task, "telegram_task", 8192, NULL, 5, NULL);
    return ESP_OK;
}

static esp_err_t offline_handler(httpd_req_t *req)
{
    memset(message, 0, sizeof(message));
    strcpy(message, "Trash-E is off the line. User Assistance Required.");
    xTaskCreate(&telegram_task, "telegram_task", 8192, NULL, 5, NULL);
    return ESP_OK;
}

static esp_err_t pickup_handler(httpd_req_t *req)
{
    memset(message, 0, sizeof(message));
    strcpy(message, "Trash-E is at pickup location");
    xTaskCreate(&telegram_task, "telegram_task", 8192, NULL, 5, NULL);
    return ESP_OK;
}
...
esp_err_t start_file_server(const char *base_path)
{
    ...
    httpd_uri_t at_obstacle = {
        .uri       = "/obstacle",   // Match all URIs of type /delete/path/to/file
        .method    = HTTP_POST,
        .handler   = obstacle_handler,
        .user_ctx  = server_data    // Pass server data as context
    };
    httpd_register_uri_handler(server, &at_obstacle);

        httpd_uri_t rover_ready = {
        .uri       = "/ready",   // Match all URIs of type /delete/path/to/file
        .method    = HTTP_POST,
        .handler   = ready_handler,
        .user_ctx  = server_data    // Pass server data as context
    };
    httpd_register_uri_handler(server, &rover_ready);

    httpd_uri_t rover_offline = {
        .uri       = "/off",   // Match all URIs of type /delete/path/to/file
        .method    = HTTP_POST,
        .handler   = offline_handler,
        .user_ctx  = server_data    // Pass server data as context
    };
    httpd_register_uri_handler(server, &rover_offline);

    httpd_uri_t at_pickup = {
        .uri       = "/pickup",   // Match all URIs of type /delete/path/to/file
        .method    = HTTP_POST,
        .handler   = pickup_handler,
        .user_ctx  = server_data    // Pass server data as context
    };
    httpd_register_uri_handler(server, &at_pickup);
